...
#raw events+ratios
overall_each_site_coarse_analysis_one<-all_life_table_data_fixed %>% 
...
#final values
overall_each_site_coarse_analysis_extrapolated<-overall_each_site_coarse_analysis_one %>%
  group_by(id,IndexCancer,Stage,Fix) %>% 
...
#arranged in a table
table_each_site_coarse_analysis_extrapolated<-overall_each_site_coarse_analysis_extrapolated %>%
...
table_each_site_coarse_early_stage<-overall_each_site_coarse_analysis_extrapolated %>%
  select(id,IndexCancer,Stage,Fix,extrapolated_outcome) %>%
...
write_tsv(overall_each_site_coarse_analysis_extrapolated,sprintf("reports/%s_MS_each_site_coarse_analysis_extrapolated.tsv",date_code))
write_tsv(table_each_site_coarse_analysis_extrapolated,sprintf("reports/%s_MS_table_each_site_coarse_analysis_extrapolated.tsv",date_code))
write_tsv(table_each_site_coarse_early_stage,sprintf("reports/%s_MS_table_each_site_coarse_early_stage.tsv",date_code))
