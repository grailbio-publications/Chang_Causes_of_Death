...
age_stratum_fix<-tribble(~Stratum,~Fix,
                         "Alive","Alive",
                         "Index Cancer","Index",
                         "Non-Cancer","NC",
                         "Other Cancer","NIndex")

overall_age_analysis_one<-all_life_table_data_fixed %>% 
  semi_join(age_id_df) %>%
  filter(IndexCancer=="All Types") %>%
  semi_join(age_stratum_fix) %>%
  left_join(age_stratum_fix) %>%
  select(-Stratum) %>%
...
#final values
overall_age_analysis_extrapolated<-overall_age_analysis_one %>%
  group_by(id,IndexCancer,Stage,Fix) %>% 
...
#arranged in a table
table_age_analysis_extrapolated<-overall_age_analysis_extrapolated %>%
...
table_age_coarse_early_stage<-overall_age_analysis_extrapolated %>%
  select(id,IndexCancer,Stage,Fix,extrapolated_outcome) %>%
...
write_tsv(overall_age_analysis_extrapolated,sprintf("reports/%s_MS_overall_age_analysis_extrapolated.tsv",date_code))
write_tsv(table_age_analysis_extrapolated,sprintf("reports/%s_MS_table_age_analysis_extrapolated.tsv",date_code))
write_tsv(table_age_coarse_early_stage,sprintf("reports/%s_MS_table_age_coarse_early_stage.tsv",date_code))
