...
overall_each_site_fine_analysis_extrapolated<-overall_each_site_fine_analysis_one %>%
  group_by(id,IndexCancer,Stage,Fix) %>% 
...
table_each_site_fine_analysis_extrapolated<-overall_each_site_fine_analysis_extrapolated %>%
  ...
  mutate(Stage=case_when(Stage=="?" ~ "Unknown/missing",
                         TRUE ~ Stage)) %>% 
  arrange(Site,Stage,Status,desc(Percent_Excluding_Index))

write_tsv(overall_each_site_fine_analysis_extrapolated,sprintf("reports/%s_MS_overall_each_site_fine_analysis_extrapolated.tsv",date_code))
write_tsv(table_each_site_fine_analysis_extrapolated,sprintf("reports/%s_MS_table_each_site_fine_analysis_extrapolated.tsv",date_code))
