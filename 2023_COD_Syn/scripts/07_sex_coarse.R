#Analysis:
#  All sites combined, excluding prostate, other male, breast, uterus, ovary, cervix, other female
#Men & women
#Stage I-IV
...
sex_id_df<-tribble(~id,
                   "Men",
                   "Women")

#coding change between sheets - note name-space collision for Other Cancer!
sex_stratum_fix<-tribble(~Stratum,~Fix,
                         "Alive","Alive",
                         "Index Cancer","Index",
                         "Non-Cancer","NC",
                         "Other Cancer","NIndex")

overall_sex_analysis_one<-all_life_table_data_fixed %>% 
  semi_join(sex_id_df) %>%
  filter(IndexCancer=="All Types") %>%
  semi_join(sex_stratum_fix) %>%
  left_join(sex_stratum_fix) %>%
  select(-Stratum) %>%
...
#final values
overall_sex_analysis_extrapolated<-overall_sex_analysis_one %>%
  group_by(id,IndexCancer,Stage,Fix) %>% 
...
#arranged in a table
table_sex_analysis_extrapolated<-overall_sex_analysis_extrapolated %>%
...
table_sex_coarse_early_stage<-overall_sex_analysis_extrapolated %>%
  select(id,IndexCancer,Stage,Fix,extrapolated_outcome) %>%
...
write_tsv(overall_sex_analysis_extrapolated,sprintf("reports/%s_MS_overall_sex_analysis_extrapolated.tsv",date_code))
write_tsv(table_sex_analysis_extrapolated,sprintf("reports/%s_MS_table_sex_analysis_extrapolated.tsv",date_code))
write_tsv(table_sex_coarse_early_stage,sprintf("reports/%s_MS_table_sex_coarse_early_stage.tsv",date_code))
